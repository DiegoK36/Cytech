...
const Login = () => {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
          try {
            const response = await fetch('http://localhost:3001/api/registro', { //usar la ruta correcta,
              headers: {
          ...
          <div className="register-container">
            <form className="register-form" onSubmit={handleSubmit}>
              <div className='volver-container'>
                ...
                <h2> Inicia Sesión en <span className="title">Cytech</span></h2>
              </div>
              ...
              <button type="submit" className='bn634-hover bn34'>Iniciar Sesión</button>
              <div className="login-link">
                ¿No tienes cuenta? <a href="/registro" className="login-link-a">Regístrate aquí.</a>
              </div>
            </form>
          </div >
          {showErrors && (
